...
const createVNPayRouter = () => {
  ...
  router.get("/vnpay/check-payment", (req, res) => {
    ...
    try {
      ...
      return res.redirect(`${FRONTEND_URL}/?vnpay_status=error`);
    }
    ...
    if (!verify.isVerified) {
      ...
      if (txnRef) {
        paymentStatuses[txnRef] = { txnRef, status: "failed", reason: "invalid_signature" };
      }
      return res.redirect(`${FRONTEND_URL}/?vnpay_status=invalid&vnpay_txnRef=${encodeURIComponent(txnRef || "")}`);
    }
    ...
    return res.redirect(
      `${FRONTEND_URL}/?vnpay_status=${status}&vnpay_txnRef=${encodeURIComponent(txnRef || "")}`
    );
  });
